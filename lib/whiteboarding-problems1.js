...
function divisibleByThreePairSum(array) {
    ...
    */
    ...
    const multipleOfThree = (max, multiple=0) => {
        // We will iterate up or equal to max.
        // Make sure to push result into memo
        // This will return a boolean

        if (multiple === max) return true;
        if (multiple > max) return false;
        multiple += 3;

        return multipleOfThree(max, multiple);
    }
